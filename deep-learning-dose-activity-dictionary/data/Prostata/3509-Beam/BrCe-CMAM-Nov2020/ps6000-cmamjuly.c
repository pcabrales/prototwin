...
{
  	uint32_t i, j,k,iw;
	uint32_t sampleCount= 12560000; /*  Make sure buffer large enough to collect data on each iteration */
	FILE * fp;
	int16_t * buffers[4];
	int16_t * appBuffers[4]; // Application buffers to copy data into
	float * gauss[4]; // Application buffers to copy data into
        float aka2;
        float  max[4];
        double timestamp;
	PICO_STATUS status;
...
// para el contador de pulsos en C
        ...
        uint64_t npulses[4],npulses_old[4];
        uint32_t idown[4],idown_old[4];
        uint32_t ip[4];
        uint32_t iup[4];
        uint32_t is;
        uint32_t icc;

        int16_t iprint[4]; // control de impresion
	...
	BUFFER_INFO bufferInfo;
        ...
        printf("%i channels\n", PS6000_MAX_CHANNELS);
        printf("Select scale Time (ns)\n");
        scanf("%u",&sampleInterval);
        printf("Select Trigger (mv)\n");
        int16_t triggg;
        scanf("%hi",&triggg);
        printf("%i Trigger\n",triggg);         
        aka2=tau2*1.e3/sampleInterval;
        printf("%f Tau\n",aka2);         


        int16_t thr = mv_to_adc(triggg,unit->channelSettings[0].range);

	for (i = PS6000_CHANNEL_A; (int32_t)i < 4; i++) // create data buffers
	{
		...
		gauss[i] = (float*) calloc(sampleCount, sizeof(double));
	}
...
//PRINTING
       ...
       //binary_out_file = fopen("/dev/null", "wb");  // w for write, b for binary
        //out_file = fopen("out.txt", "w");  // w for write, b for binary
...
//        tt_file[0] = fopen("t0.txt", "w");  // w for write, b for binary
//        tt_file[1] = fopen("t1.txt", "w");  // w for write, b for binary
//        tt_file[2] = fopen("t2.txt", "w");  // w for write, b for binary
//        tt_file[3] = fopen("t3.txt", "w");  // w for write, b for binary
...
//			printf("\nCollected %3li samples, index = %5lu, Total: %6d samples ", g_sampleCount, g_startIndex, totalSamples);
			...
			if (g_trig)
			{
				printf("Trig. at index %u Total at trigger: %lu", triggeredAt, previousTotal + (triggeredAt - g_startIndex + 1) );	// show where trigger occurred
			}
...
//retorna_analyza:
            ...
            //                 fprintf(test_out_file," #real y ac. live time %.12f %.12f  %i\n",time_real,time_live,npulses,g_startIndex);
            //                 fprintf(test_out_file,"\n");

                            		for (kkk=0;kkk<4;kkk++)idown_old[kkk]=0;
                       		}    
                        	printf(" %.2f %8.2f %8.2f  %li %8.2f %.6f\n",total_r,(timett-tlive)/timett*100,(total_r-total_live)/total_r*100,g_sampleCount,totalSamples*sampleInterval*1.e-9,total_r-total_live);
                                //iw=iw+1;
    //                            printf("buff remain, buff ini%i %i \n",sampleCount-g_startIndex,g_startIndex);
...
//
                                ...
                                float ymax[4];
				float max2[4];
                             	double width_min=300.;  //3 us minimum width of pulse
                             	double sumint=0.;
                             	double sumnn=0.;
                             	double baseline=0.;                                 
                             	uint64_t is2,ipul;
// Comienza el analisis de pulsos para integrar energias y contar pulsos. 
				...
				for (icc2=0; icc2 < 4; icc2++)   //bucle en canal
                           	{  
                               		sumint=0.;
                               		sumnn=0.;
                               		baseline=0.;
                               		...
                               		{
                            	   		//appBuffers[icc2][is2]=-(appBuffers[icc2][is2]);
						gauss[icc2][is2]=0;
                               		}
                              		...
                              		{
                                   		// buscando triggers
                                   		// incluir aqui calculo baseline
                                 		sumint=sumint+appBuffers[icc2][is2]-baseline;
                                		if ( npulses[icc2] % 10000 == 10 && iprint[icc2]==1  )
						{
							ipul=0;
                                    			for (ikkk=idown[icc2]-6 ; ikkk < iup[icc2]+10; ikkk++)
                                    			{
								ipul++;	 
                                    				//fprintf(pulsesfile[icc2]," %.12f  %i \n", timelive_old+(ipul*1.e-9*sampleInterval),appBuffers[icc2][ikkk]);
                                    				fprintf(pulsesfile[icc2]," %.12f  %i %f\n", (ipul*1.e-9*sampleInterval),appBuffers[icc2][ikkk],gauss[icc2][ikkk]);
                                     			} 
                                    			fprintf(pulsesfile[icc2],"\n");
                                    			iprint[icc2]=0; 
		//                                    fflush(pulsesfile[icc2]);
                                		 }   // end test pulses

// esto se puede refinar, para hacer solo una suma en cada muestra
// se copia de lo que hice en fortran (JMU), pero otro dia...
//
// ip==0, no tenemos pulso, promediamos las 6 muestras anteriores
                      				if(ip[icc2]==0 && is2 > 10 ) baseline=(appBuffers[icc2][is2-11]+appBuffers[icc2][is2-10]+appBuffers[icc2][is2-9]+appBuffers[icc2][is2-8]+appBuffers[icc2][is2-7]+appBuffers[icc2][is2-6])/6;

                                 		if( (appBuffers[icc2][is2] > thr) && ip[icc2]==0 )
                                  		{
                                        		max[icc2]=-10000.;
							ip[icc2]=1;idown[icc2]=is2;sumint=(appBuffers[icc2][is2]-baseline)/2.;
                                  			for(igg = idown[icc2]-6;igg < idown[icc2]+20;igg++)
                                                        { 
					        	//gauss[icc2][igg]=((aka2+2*aka2*aka2)*gauss[icc2][igg-1]-aka2*aka2*gauss[icc2][igg-2]+2*cr[icc2][igg])/(1.+aka2+aka2*aka2);
					        		gauss[icc2][igg]=((aka2+2*aka2*aka2)*gauss[icc2][igg-1]-aka2*aka2*gauss[icc2][igg-2]+2*appBuffers[icc2][igg])/(1.+aka2+aka2*aka2);
                                              	        	if(gauss[icc2][igg] > max[icc2])
                                                        	{
									max[icc2]=gauss[icc2][igg];
                                                                	imax[icc2]=igg;
                                                        	}
							}
							max2[icc2]=0;
							for(ig = imax[icc2]-2; ig < imax[icc2]+3;ig++)
							{
							max2[icc2]+=gauss[icc2][ig];
							}
							max2[icc2]=max2[icc2]*0.2;
							//x1=-1;
							//x2=0;
							//x3=1;
                                                        y1=gauss[icc2][imax[icc2]-1];
                                                     	y2=gauss[icc2][imax[icc2]];
                                                      	y3=gauss[icc2][imax[icc2]+1];
							ymax[icc2]=((y3-y1)*(y3-y1))/(16*(y3+y1)/2-y2)+y2;
						}
                                                //if( ip[icc2]==1)
                                                //{ 
					        //	gauss[icc2][is2]=((aka2+2*aka2*aka2)*gauss[icc2][is2-1]-aka2*aka2*gauss[icc2][is2-2]+2*cr[icc2][is2])/(1.+aka2+aka2*aka2);
                                  			//sumint=sumint+appBuffers[0][is]-baseline;
                                              	//        if(gauss[icc2][is2] > max[icc2])max[icc2]=gauss[icc2][is2];
						//}
                 //                   for (is2=idown-5;idown;is2++){baseline=baseline+appBuffers[0][is2]/5;}
                                   	         //calculada baselinea en este trigger
                                 		if( (appBuffers[icc2][is2] < thr)  && ip[icc2]==1 )
                                   		{                                  
                                    			ip[icc2]=0;
                                    			iup[icc2]=is2;
                                    			sumint=sumint-(appBuffers[icc2][is2]-baseline)/2.;
                                    			sumnn=sumint/(iup[icc2]-idown[icc2]);
                                    			sumint=0;
                                    			if( (iup[icc2]-idown[icc2])*sampleInterval > width_min )
                                			{ 
                                    				if( iprint[icc2]==-100)
                                    				{  
                                    					fprintf(test_out_file2," %i  %.5f",icc2,timelive_old+(idown[icc2]*1.e-9*sampleInterval));
                                    					if (icc2==3)fprintf(test_out_file2,"\n");
                                    				}
                                       				iprint[icc2]=1; // un pulso mas, ya se puede imprimir
                                        			npulses[icc2] ++ ;
                //             fprintf(test_out_file,"#np wd ic is t E\n");
                              					//fprintf(out_file," %.8i %i %i %i %.12f  %.5f %.5f \n", npulses[icc2],iup[icc2]-idown[icc2],icc2,idown[icc2]-idown_old[icc2],timelive_old+(idown[icc2]*1.e-9*sampleInterval),sumnn,max[icc2]);
                						//myData.icc = icc2;
								timestamp = timelive_old+(idown[icc2]*1.e-9*sampleInterval);
								//myData.maxValue = max[icc2];
								//fwrite(&num, sizeof(struct threeNum), 1, fptr)
								fwrite(&icc2,sizeof(int16_t),1,binary_out_file);
								fwrite(&timestamp,sizeof(double),1,binary_out_file);
								fwrite(&max[icc2],sizeof(float),1,binary_out_file);
								fwrite(&max2[icc2],sizeof(float),1,binary_out_file);
								fwrite(&ymax[icc2],sizeof(float),1,binary_out_file);
                           		...
                           		fprintf(test_out_file2,"  %.3f %li  ",(npulses[icc2]-npulses_old[icc2])*1./tlive,npulses[icc2]-npulses_old[icc2]); 
                                  	printf("  %.3f %i  \n",(npulses[icc2]-npulses_old[icc2])*1./tlive,npulses[icc2]-npulses_old[icc2]); 
...
//                                  fflush(test_out_file2);
                             		...
                             		//fprintf(test_out_file,"# CHAN (%i) Parameters Bad %.12f %i",icc2,timelive_old,g_sampleCount);
                             		fprintf(test_out_file,"\n");
