...
void StreamDataHandler(UNIT * unit, uint32_t preTrigger)
{
    ...
    float * gauss[channels];
    
    int16_t  max;
    float  y1,y2,y3;
    float  ymax;
    float tau=0.1;
    float aka2=tau*1.e3/sampleInterval;
...
// para el contador de pulsos en C
	...
	for (i = PS6000_CHANNEL_A; (int32_t)i < channels; i++) // create data buffers
	{
		...
		appBuffers[i] = (int16_t*) calloc(sampleCount, sizeof(int16_t));
                gauss[i] = (float*) calloc(sampleCount, sizeof(float)); 
	}
...
//PRINTING
	...
	while (!_kbhit() && !g_autoStopped && (timerTime > total_r || timerTime == 0.) ) // streaming mientras no se toca teclado o no se para solo
	{
		...
		usleep(1000000);  // tenemos una espera de al menos un segundo antes de preguntar por los datos
...
//
                ...
                double width_min=400.;  //3 us minimum width of pulse
                float sumint=0.;
                float sumnn=0.;
                double baseline=0.;                                 
                uint64_t ipul;
// Comienza el analisis de pulsos para integrar energias y contar pulsos. 
                    ...
                    for (is2=g_startIndex; is2 < g_startIndex+g_sampleCount-1; is2++ )
                    {
                        ...
                        {   
                            ...
                            for (ikkk=idown[icc2]-6 ; ikkk < iup[icc2]+20; ikkk++)
                                {
                                    ipul++;	 
                                    fprintf(pulsesfile[icc2]," %.12f  %i %f %f\n", (ipul*1.e-9*sampleInterval),appBuffers[icc2][ikkk],gauss[icc2][ikkk],ymax);
                                } 
                        ...
                        {                                  
                            ...
                            {
                                max=0;
                                for (is3 = idown[icc2]; is3 < iup[icc2]+10;is3++)
                                {
				gauss[icc2][is3]=((aka2+2*aka2*aka2)*gauss[icc2][is3-1]-aka2*aka2*gauss[icc2][is3-2]+2*appBuffers[icc2][is3])/(1.+aka2+aka2*aka2);
                                    if(gauss[icc2][is3] > max)
                                    {
                                        max=gauss[icc2][is3];
                                        imax[icc2]=is3;
                                    }                                    
				...
				float A1=0.5*(y3+y1-2*y2);
				float B1=0.5*(y3-y1);
				float C1=y2;
                                ymax = C1-0.25*B1*B1/A1;
	 			//ymax=((y3-y1)*(y3-y1))/(16*(y3+y1)/2-y2)+y2;
                                //ymax=(y1-y3)*0.5+(y3-y1)*(y1-y3)/(y1+y3-2*y2)+y2*(y1-y3)/(y1+y3-2*y2);
                                iprint[icc2]=1; // un pulso mas, ya se puede imprimir
                                npulses[icc2] ++ ;
				timestamp = timelive_old+(iup[icc2]*1.e-9*sampleInterval);
				...
				{
                    fprintf(test_out_file2," %.8i %.4i %.12f  \n", npulses[icc2],icc2,timelive_old+(iup[icc2]*1.e-9*sampleInterval));
                }
	...
	if (unit->handle) 
	{
		...
		switch (variant)
		{
			...
			case MODEL_PS6403D:
				...
				for (i = 0; i < PS6000_MAX_CHANNELS; i++)
				{
					unit->channelSettings[i].range = PS6000_500MV;
					unit->channelSettings[i].DCcoupled = PS6000_DC_50R;
					//unit->channelSettings[i].DCcoupled = PS6000_DC_1M;
					unit->channelSettings[i].enabled = TRUE;
