                    ...
                    for (is2=g_startIndex; is2 < g_startIndex+g_sampleCount-1; is2++ )
                    {
                        ...
                        {                                  
                            ...
                            if( (iup[icc2]-idown[icc2])*sampleInterval > width_min )
                            {
                                sumint = 0;
                                baseline = 0;
                                for (is3 = idown[icc2]-20; is3 < idown[icc2]-10;is3++)
                                {
                                    if(is3 > g_startIndex)
                                    {
                                        baseline += appBuffers[icc2][is3];
                                        ibase ++;
                                    }
                                }
                                baseline = baseline/ibase;
                                for (is3 = idown[icc2]-10; is3 < iup[icc2];is3++)
                                {
                                    if(is3 > g_startIndex && is3 < g_startIndex+g_sampleCount-1)
                                    {
                                        sumint=sumint+(appBuffers[icc2][is3]+appBuffers[icc2][is3])*0.5-baseline;
                                        isum ++;
                                    }
                                }
                                sumnn =sumint/isum;
/*                                    if(appBuffers[icc2][is3] > max)
                                ...
                                y1=appBuffers[icc2][imax[icc2]-1];
                                y2=appBuffers[icc2][imax[icc2]];
                                y3=appBuffers[icc2][imax[icc2]+1];
				...
				fwrite(&timestamp,sizeof(double),1,binary_out_file);
                                fwrite(&sumnn,sizeof(float),1,binary_out_file);
